...
const handleDecision = (markedSafe, isTimeout = false) => {
  ...
  if (isTimeout) {
    ...
    // Save timeout as incorrect attempt
    if (user.value) {
      $fetch('/api/attempts', {
        method: 'POST',
        body: {
          scenario_id: currentEmail.value.id,
          user_email: user.value.email || user.value.userInfo?.email || 'unknown',
          is_correct: false
        }
      }).catch(err => console.error('Failed to save timeout stats', err));
    }
    ...
    // --- SAVE ATTEMPT TO DB ---
    if (user.value) {
      // Best effort - don't await/block UI
      $fetch('/api/attempts', {
        method: 'POST',
        body: {
          scenario_id: currentEmail.value.id,
      ...
      <div v-if="!gameStarted" class="welcome-wrapper">
        <div class="welcome-card">
          ...
          <div class="features-grid">
            <div class="feature-box"><span class="f-title">🧭 Decide</span><span class="f-divider">|</span><span
                class="f-desc">Accept or flag each email</span></div>
            <div class="feature-box"><span class="f-title">💡 Learn instantly</span><span
                class="f-divider">|</span><span class="f-desc">Quick feedback after every choice</span></div>
            <div class="feature-box"><span class="f-title">📈 Score & progress</span><span
                class="f-divider">|</span><span class="f-desc">Streaks, levels, harder attacks</span></div>
          </div>
    ...
    <div v-if="gameStarted" class="game-ui">
      ...
      <div class="stats-container">
        ...
        <div class="lives-wrapper">
          <HeartIcon v-for="n in 5" :key="n" class="heart-icon" :class="[
            { 'lost': n > lives },
            heartAnimations[n - 1]
          ]" />
        </div>
        ...
        <div class="score-wrapper">
          <div class="score-item">
            <CheckCircleIcon class="score-icon correct-color" /><span class="score-text">{{ correctCount }}</span>
          </div>
          <div class="score-item">
            <XCircleIcon class="score-icon incorrect-color" /><span class="score-text">{{ incorrectCount }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="mail-container">
          ...
          <TransitionGroup name="email-list" tag="div" class="email-items-wrapper">
            <div v-for="email in emails" :key="email.id" class="email-item" :class="{
              ...
              <div class="email-details">
                <div class="email-top-row"><span class="email-sender">{{ email.sender }}</span><span
                    class="email-date">{{ email.date }}</span></div>
        ...
        <div class="message-preview">
          ...
          <div v-else-if="error" class="empty-state">
            ...
            <p>{{ error }}</p><button @click="fetchScenarios" class="primary-btn">Retry</button>
          </div>
          ...
          <div v-else-if="currentEmail" class="email-content-wrapper">
            ...
            <div class="email-action-footer">
              <button @click="handleDecision(true)" class="action-btn safe-btn" :disabled="isProcessingDecision">
                <ShieldCheckIcon class="btn-icon" /> Mark Safe
              </button>
              <button @click="handleDecision(false)" class="action-btn phish-btn" :disabled="isProcessingDecision">
      ...
      <div v-if="showHelpModal" class="modal-overlay" @click.self="toggleHelpModal">
        <div class="modal-card">
          ...
          <div class="modal-content">
            ...
            <div class="modal-footer"><button @click="toggleHelpModal" class="primary-btn return-btn">RETURN TO
                GAME</button></div>
          </div>
        </div>
      </div>
...
:deep(.email-body-content > *) {
  margin-left: 0;
  margin-right: 0;
  padding-left: 0;
}

:deep(.email-body-content p) {
  margin-bottom: 15px;
  margin-top: 0;
}

:deep(.email-body-content ul),
:deep(.email-body-content ol) {
  margin-left: 20px;
  padding-left: 0;
}
